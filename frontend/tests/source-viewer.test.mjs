...
globalThis.URL.createObjectURL = () => 'blob:test-pdf';
let revokedUrl = null;
...
assert.match(groupedMarkup, /Trang 6/);
assert.doesNotMatch(groupedMarkup, /\[C2, C4\]/);

await showPagePreview('sample.pdf', 7, 'doc-1');

assert.equal(calls[0].url, '/api/documents/doc-1/file');
assert.equal(calls[0].headers.Authorization, 'Bearer viewer-token');
assert.equal(embed.src, 'blob:test-pdf#page=7');
...
assert.equal(revokedUrl, 'blob:test-pdf');
